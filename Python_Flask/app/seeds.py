...
# First, it drops all the existing tables. Second, it creates any tables that Base mapped,
# in a class that inherits Base (like User).
...
# Note that db.add_all() alone doesn't change the database.
# In fact, it only preps the SQL queries. To run the INSERT statements, you need to call db.commit().
